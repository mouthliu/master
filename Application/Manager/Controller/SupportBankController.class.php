...
/**
 * Class SupportBankController
 * @package Manager\Controller
 * 支持银行表控制器
 */
class SupportBankController extends BaseController {
    public function getUpdateRelation()
    {
        $order = M('SupportBank') ->where(array('status'=>array('neq',9))) ->order('create_time ASC') ->select();
    ...
    function derive()
    {
        ...
        if(!empty($_REQUEST['s_time'])&&!empty($_REQUEST['e_time'])){
            ...
            $where['s_bank.create_time'] = array('between',"$start_time, $end_time");
        }elseif(!empty($_REQUEST['s_time'])){
            $where['s_bank.create_time'] = array('egt',strtotime($_REQUEST['s_time']));
        }elseif(!empty($_REQUEST['e_time'])){
            $where['s_bank.create_time'] = array('elt',strtotime($_REQUEST['e_time']));
        }else{

        }
        $model = M('SupportBank') ->alias('s_bank')
            ->field('s_bank.*,member.nickname as m_nickname,master.nickname as master_nickname')
            ->join(array(
                'LEFT JOIN '.C('DB_PREFIX').'member member ON member.id = s_bank.m_id',
                'LEFT JOIN '.C('DB_PREFIX').'master master ON master.id = s_bank.master_id',
            ))
            ->order('s_bank.create_time desc')
            ->select();
