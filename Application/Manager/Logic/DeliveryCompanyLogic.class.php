 ...
 * 快递公司逻辑层
 */
class DeliveryCompanyLogic extends BaseLogic {
    ...
    function getList($request = array()){
        ...
        $param['where']['d_company.status'] = array('neq',9);
        $param['order'] = 'd_company.create_time DESC';   //排序
        $param['page_size'] = C('LIST_ROWS');      //页码
        $param['parameter']         = $request;
        $result = D('DeliveryCompany')->getList($param);
        return $result;
    }
    ...
    function findRow($request = array()){
        if(!empty($request['id'])) {
            $param['where']['d_company.id'] = $request['id'];
        } else {
            $this->setLogicError('参数错误！'); return false;
        }
        $row = D('DeliveryCompany')->findRow($param);
