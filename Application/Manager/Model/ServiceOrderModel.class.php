...
class ServiceOrderModel extends BaseModel {
    ...
    function getList($param = array()) {
        if(!empty($param['page_size'])) {
            $total      = $this ->alias('s_order') ->where($param['where'])
                ->join(array(
                    'LEFT JOIN '.C('DB_PREFIX').'member member ON member.id = s_order.m_id',
                    'LEFT JOIN '.C('DB_PREFIX').'coupon coupon ON coupon.id = s_order.coupon'
                ))
                ->count();
            $Page       = $this ->getPage($total, $param['page_size'], $param['parameter']);
            $page_show  = $Page ->show();
        }
        $model  = $this ->alias('s_order') ->where($param['where'])
            ->join(array(
                'LEFT JOIN '.C('DB_PREFIX').'member member ON member.id = s_order.m_id',
                'LEFT JOIN '.C('DB_PREFIX').'coupon coupon ON coupon.id = s_order.coupon'
            ))
            ->field('s_order.*,member.nickname,coupon.id as c_id')
            ->order($param['order']);
    ...
    function findRow($param = array()) {
        $row = $this ->alias('s_order') ->where($param['where'])
            ->join(array(
                'LEFT JOIN '.C('DB_PREFIX').'member member ON member.id = s_order.m_id',
                'LEFT JOIN '.C('DB_PREFIX').'master master ON master.id = s_order.master_id',
            ))
            ->field('s_order.*, member.nickname as m_nickname,master.nickname as master_nickname')
            ->find();
        return $row;
    }
