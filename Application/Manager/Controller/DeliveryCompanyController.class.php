...
class DeliveryCompanyController extends BaseController {
    ...
    {
        $order = M('DeliveryCompany') ->where(array('status'=>array('neq',9))) ->order('create_time ASC') ->select();
    ...
    function derive()
    {
        ...
        if(!empty($_REQUEST['s_time'])&&!empty($_REQUEST['e_time'])){
            ...
            $where['sign.create_time'] = array('between',"$start_time, $end_time");
        }elseif(!empty($_REQUEST['s_time'])){
            $where['sign.create_time'] = array('egt',strtotime($_REQUEST['s_time']));
        }elseif(!empty($_REQUEST['e_time'])){
            $where['sign.create_time'] = array('elt',strtotime($_REQUEST['e_time']));
        }else{

        }
        $model = M('Sign') ->alias('sign')
            ->field('sign.*,member.nickname as m_nickname,master.nickname as master_nickname')
            ->join(array(
                'LEFT JOIN '.C('DB_PREFIX').'member member ON member.id = sign.m_id',
                'LEFT JOIN '.C('DB_PREFIX').'master master ON master.id = sign.master_id',
            ))
