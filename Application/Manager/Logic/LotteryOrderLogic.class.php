...
class LotteryOrderLogic extends BaseLogic {
    ...
    function getList($request = array()){
        ...
        if($request['lo_name']){
            $param['where']['lottery.lo_name'] = array('like','%'.$request['lo_name'].'%');
        }
        if($request['nickname']){
            $param['where']['member.nickname'] = array('like','%'.$request['nickname'].'%');
        }
        if($request['status'] != 999){
            $param['where']['l_order.status'] = $request['status'];
        }else{
            $param['where']['l_order.status'] = 0;
        }
        if(empty($request['status'])){
            $param['where']['l_order.status'] = array('neq',9);
        }
        $param['order'] = 'l_order.create_time DESC';   //排序
    ...
    function findRow($request = array()){
        if(!empty($request['id'])) {
            $param['where']['l_order.id'] = $request['id'];
        } else {
